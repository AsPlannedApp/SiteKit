...
export function collectSections(loadedModules, config) {
    ...
    for (const mod of loadedModules) {
        ...
        const summary = summaryFn ? summaryFn({ content: mod.content, config }) : null;
 ...
 * Extension modules' own `globalCss` is appended into the same
...
export function renderAll(loadedModules, config, { alwaysIncludeClient = () => true, themeColors = {} } = {}) {
    ...
    for (const mod of loadedModules) {
        ...
        if (mod.template && typeof mod.template.render === 'function') {
            ...
            const result = mod.template.render({ content: mod.content, config, ctx, themeColors });
